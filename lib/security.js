...
import { isDbError } from "./apiResponse";
...
export async function requireAuth(request, allowedRoles) {
  ...
  // Fail-closed: DB/network failure during this lookup must NOT be treated as valid
  if (payload.staffId) {
    try {
      ...
      // Fail-closed on DB/network error — never continue as authenticated
      // Preserve generic 401 for disabled/missing; only DB errors become 503
      if (isDbError(err)) {
