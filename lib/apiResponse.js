...
export function fail(error = "Internal Server Error", status = 500, code) {
  ...
  // Optional machine-readable `code` (e.g. SESSION_REVOKED) lets clients branch
  // without parsing message strings. Omitted when absent — contract unchanged.
  const envelope = { success: false, data: null, error: message, message };
  if (typeof code === "string" && code) envelope.code = code;
