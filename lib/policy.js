...
//
// Roles are the Staff enum (lib/models/Staff STAFF_ROLES). The authenticated
// role comes from the canonical resolver (Session -> live Staff); callers
// must never substitute cookie payload, client input, or snapshots.
//
// Usage:
...
export const ALL_ROLES = [ROLES.WAITER, ROLES.KITCHEN, ROLES.BARISTA, ROLES.MANAGER, ROLES.CASHIER];
...
export const GROUPS = {
  ...
  CASHIER_OR_MANAGER: [ROLES.CASHIER, ROLES.MANAGER],
  CASHIER_ONLY: [ROLES.CASHIER],
};
...
const MATRIX = {
  ...
  "orders:transition:PAYMENT_PENDING": [ROLES.WAITER, ROLES.MANAGER],
  "orders:transition:PAID": [ROLES.CASHIER, ROLES.MANAGER],
  "orders:transition:ARCHIVED": [ROLES.KITCHEN, ROLES.BARISTA, ROLES.MANAGER],
  "orders:transition:CANCELLED": [ROLES.MANAGER],
  "orders:cancel:item": [ROLES.KITCHEN, ROLES.BARISTA, ROLES.MANAGER],
  "orders:payment": [ROLES.WAITER, ROLES.MANAGER],
  "orders:payment:submit": [ROLES.WAITER, ROLES.MANAGER],
  "orders:payment:confirm": [ROLES.CASHIER, ROLES.MANAGER],
  "orders:payment:reject": [ROLES.CASHIER, ROLES.MANAGER],
  ...
  // Inventory (Manager only — Phase D)
  "inventory:read": [ROLES.MANAGER],
  "inventory:mutate": [ROLES.MANAGER],

  // Recipe (Manager only — Phase F)
  "recipe:read": [ROLES.MANAGER],
  "recipe:mutate": [ROLES.MANAGER],
...
// ARCH-6: getAuthorizedSession removed. It was a second authorization engine
// (direct HMAC decode, no live Staff check) with zero callers. Server
// Components/Actions must use getLiveSessionFromCookies from lib/serverAuth
// (canonical resolver: server session -> live Staff -> role).
