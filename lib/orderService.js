...
export async function updateOrderStatus(conn, identifier, status, opts = {}) {
  ...
  if (!doc) {
    ...
    if (isKitchen && !hasFood) throw new Error("Order has no active FOOD items for Kitchen");
    if (isBarista && !hasDrink) throw new Error("Order has no active DRINK items for Barista");
...
export async function cancelOrderItem(conn, identifier, lineId, opts = {}) {
  ...
  const existing = await Order.findOne(buildOrderQuery(identifier)).select("status items").lean();
  if (!existing) throw new Error("Order not found");
  ...
  // Fallback for legacy without lineId: match by _id string if provided (not recommended but safe)
  ...
  const filter = {
    ...buildOrderQuery(identifier),
    status: { $nin: ["SERVED", "PAYMENT_PENDING", "PAID", "CANCELLED", "ARCHIVED"] },
    "items.lineId": lineOid,
    "items.cancelled": { $ne: true },
  };
  // Strengthen station guard in filter for concurrency safety
  if (isKitchen) filter["items.type"] = "FOOD";
  if (isBarista) filter["items.type"] = "DRINK";
  ...
  if (!docAfter) {
    // Re-evaluate for precise error
    const re = await Order.findOne(buildOrderQuery(identifier)).select("status items").lean();
    if (!re) throw new Error("Order not found");
    if (["SERVED", "PAID", "CANCELLED", "ARCHIVED"].includes(re.status)) throw new Error("Cannot cancel line of order in terminal state");
  ...
  // Derive overall status: if no active lines remain → whole order CANCELLED, else keep truthful status.
  // Active = not cancelled. Also derive station status emptiness.
  const active = (docAfter.items || []).filter((it) => !it.cancelled);
  if (active.length === 0) {
    // All lines cancelled → mark whole order CANCELLED (distinct from ARCHIVED)
    const cancelledDoc = await Order.findOneAndUpdate(
      { ...buildOrderQuery(identifier), status: { $nin: ["CANCELLED", "PAID"] } },
      { $set: { status: "CANCELLED", updatedAt: new Date() } },
      { returnDocument: "after", runValidators: false }
    );
    return cancelledDoc || docAfter;
  }

  // If active remains, ensure overall status reflects remaining work:
  // Recompute hasFood/hasDrink from active only; if mismatched with stored kitchenStatus/baristaStatus,
  // leave them but do not auto-cancel. For mixed orders where cancelled line removes one station,
  // readiness should be derived from remaining active stations only.
  // If docAfter.status is PENDING/PREPARING/READY and active station now empty, we normalize status lazily
  // on next updateOrderStatus call. No automatic status mutation here to avoid surprising order disappearance.
  // However ensure cancelled line does not falsely satisfy readiness: already handled by active filter in reportService.
  return docAfter;
}
...
export async function submitPaymentForVerification(conn, identifier, { paymentMethod, paymentAccountId, actorId } = {}) {
  ...
  // Only SERVED (or READY for backward compat) can submit payment; preserve waiter ownership already checked in route
  const filter = {
    ...buildOrderQuery(identifier),
  ...
  if (!doc) {
    ...
    if (!["SERVED", "READY"].includes(existing.status)) throw new Error("Order must be served before payment submission");
    throw new Error("Update failed");
  }
  return doc;
}
