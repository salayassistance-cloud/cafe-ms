...
export async function getReport(conn, { from, to, prevFrom, interval }) {
  ...
  const orders = await Order.find({ createdAt: { $gte: prevFrom, $lt: to } })
    .select("status kitchenStatus baristaStatus totalAmount createdAt items waiterName waiterId waiterNumber waiterInfo kitchenStaffId baristaStaffId paymentMethod paymentAccountId paymentAccountSnapshot preparingAt readyAt servedAt paidAt completedAt kitchenPreparingAt kitchenReadyAt baristaPreparingAt baristaReadyAt updatedAt isExternal")
    .sort({ createdAt: 1 })
