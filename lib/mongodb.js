...
async function syncIndexesInBackground(conn) {
  ...
  try {
    ...
    const { getInventoryAuditModel } = await import("./models/InventoryAudit");
    // AUTH-ARCH-8F: SystemAuth is retired as runtime authority (model file
    // retained for explicit ops scripts only). It is intentionally excluded
    // from background index sync — existing indexes are untouched, no data
    // writes occur here, and scripts import the model directly when needed.
    await Promise.all([
      ...
      getInventoryAuditModel(conn).syncIndexes(),
    ]);
  } catch (error) {
